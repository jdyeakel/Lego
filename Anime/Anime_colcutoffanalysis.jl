...
@time @sync @parallel for r=1:reps
    ...
    namespace_rep = string("$(homedir())/2014_Lego/Anime/data/colcutoff/int_m",r,".jld");
    ...
    namespace_cid = string("$(homedir())/2014_Lego/Anime/data/colcutoff/cid_",r,".jld");
...
plot(seq(1,$tmax),$meansp,type='l',ylim=c(0,275))
polygon(c(seq(1,$tmax),rev(seq(1,$tmax))),c($meansp+$stdsp, rev($meansp-$stdsp)),col='#7F7F7F50',border=NA)
dev.off()
"""
