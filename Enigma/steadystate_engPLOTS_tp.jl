...
# namespace = "$(homedir())/2014_Lego/Enigma/data/steadystate_eng/sim_settings.jld";
...
SSL = mean(SS_indlinkdensity);

@sync @distributed for r=1:reps
    #Read in the interaction matrix
    
    # if homedir() == "/home/z840"
    #     namespace = string("$(homedir())/2014_Lego/Enigma/data/steadystate_eng/int_m",r,".jld");
    # else
    #     namespace = string("$(homedir())/Dropbox/PostDoc/2014_Lego/Enigma/data/steadystate_eng/int_m",r,".jld");
    # end
    filename = "data/steadystate_eng/int_m.jld";
    indices = [r];
    namespace = smartpath(filename,indices);
    ...
    # 
    # if homedir() == "/home/z840"
    #     namespace = string("$(homedir())/2014_Lego/Enigma/data/steadystate_eng/cid_",r,".jld");
    # else
    #     namespace = string("$(homedir())/Dropbox/PostDoc/2014_Lego/Enigma/data/steadystate_eng/cid_",r,".jld");
    # end
    ...
    for t = 1:tseqmax
        ...
        realizeddegree[r,t,1:length(spcid)] = vec(sum(a_b[spcid,[1;cid]],dims=2));
        potentialdegree[r,t,1:length(spcid)] = vec(sum(a_b[spcid,:],dims=2));
        ...
        cidinweb = sort(spcid[findall(!iszero,vec(sum(tp_m[spcid,[1;spcid]],dims=2)))]);
        
        sprichinweb[r,t] = length(cidinweb);
        
        C[r,t] = (sum(tind_m[cidinweb,[1;spcid]]))/((length(cidinweb))^2);
        
        #manuscript measure
        # realizedG[r,t,1:length(cidinweb)] = vec(sum(a_b[cidinweb,[1;cid]],dims=2)) .* (1/((sum(a_b[cidinweb,[1;cid]]))/(length(cidinweb))));
        realizedG[r,t,1:length(cidinweb)] = vec(sum(tind_m[cidinweb,[1;spcid]],dims=2)) .* (1/((sum(tind_m[cidinweb,[1;spcid]]))/(length(cidinweb))));
        
        potentialG[r,t,1:length(cidinweb)] = vec(sum(a_b[cidinweb,:],dims=2)) .* (1/((sum(a_b[cidinweb,[1;cid]]))/(length(cidinweb))));
        
        # cidinwebnoprim = sort(spcid[findall(!iszero,vec(sum(a_b[spcid,cid],dims=2)))]);
        ...
        if sprichinwebnoprim[r,t] > 0
            ...
            realizedGnoprim[r,t,1:length(cidinwebnoprim)] = vec(sum(tind_m[cidinwebnoprim,spcid],dims=2)) .* (1/((sum(tind_m[cidinwebnoprim,spcid]))/(length(cidinwebnoprim))));
        end
        ...
        if sprichinwebnoprim[r,t] > 0
            ...
            realizedGavgc[r,t,1:length(cidinweb)] = vec(sum(tp_m[cidinweb,spcid],dims=2)) .* (1/(SSL));
            potentialGavgc[r,t,1:length(cidinweb)] = vec(sum(tp_m[cidinweb,2:S],dims=2)) .* (1/(SSL));
        end
...
filename = "figures/yog/fig_trophic2_eng_tp.pdf";
namespace = smartpath(filename);
R"""
...
namespace = smartpath(filename,indices);
@load namespace CID clock;
speciesrich = sum(CID[1:200,:],dims=1);
...
plot(trimdist,seq(1,length(trimdist)),type='l',xlim=c(0,0.6),ylim=c(1,20),col=pal[1],xlab='',ylab='',axes=FALSE)
axis(2,at=seq(2,20,by=2),labels=TRUE,tck=-0.015,mgp=c(0.5,0.5,0),las=1)
...
R"""
legend(x=0.45,y=20.5,legend=$(seq[seq2]),pt.cex=1.2,pt.bg=pal,col='black',pch=21,bty='n',title='',cex=0.8)
text(0.45,19.7,'Assembly time')
dev.off()
"""
