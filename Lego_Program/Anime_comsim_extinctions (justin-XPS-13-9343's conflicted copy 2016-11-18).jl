...
for i=1:ln
  for r=1:reps
    ...
    namespace = string("$(homedir())/Dropbox/PostDoc/2014_Lego/Lego_Program/data/comsim_ext/comgen_n/sparsecg_i",i,"_r",r,".jld");
    save(namespace,"sm",sparsecg);
  end
end
#The rest of the data
save("$(homedir())/Dropbox/PostDoc/2014_Lego/Lego_Program/data/comsim_ext/overn.jld","INT",INT,"rve",RvsE,"rvesd",RvsEsd,"rich",rich_ss);
...
for i=1:ln
  for r=1:reps
    sparsecg = sparse(COMGEN[i][r,:,:]);
    namespace = string("$(homedir())/Dropbox/PostDoc/2014_Lego/Lego_Program/data/comsim_ext/comgen_m/sparsecg_i",i,"_r",r,".jld");
    save(namespace,"sm",sparsecg);
  end
end
#Save Data
save("$(homedir())/Dropbox/PostDoc/2014_Lego/Lego_Program/data/comsim_ext/overm.jld","rve",RvsE,"rvesd",RvsEsd,"rich",rich_ss,"comgen",COMGEN);
...
d = load("$(homedir())/Dropbox/PostDoc/2014_Lego/Lego_Program/data/comsim_ext/overn.jld");
...
d = load("$(homedir())/Dropbox/PostDoc/2014_Lego/Lego_Program/data/comsim_ext/overm.jld");
#This loads the dictionary
RvsE = d["rve"];
rich_ss = d["rich"];
...
#Plot Extinctions vs. Richness mean need
R"""
library(RColorBrewer);
cols <- brewer.pal(10,'Spectral');
x=$(RvsE[1][:,1]);
y=$(RvsE[1][:,2]);
pdf('$(homedir())/Dropbox/PostDoc/2014_Lego/Lego_Program/figures/fig_rveM.pdf',width=5,height=4)
...
#Plot Extinctions vs. Richness SD need
R"""
library(RColorBrewer,lib.loc="/usr/local/lib/R/site-library/");
cols <- brewer.pal(10,'Spectral');
x=$(RvsEsd[1][:,1]);
y=$(RvsEsd[1][:,2]);
pdf('$(homedir())/Dropbox/PostDoc/2014_Lego/Lego_Program/figures/fig_rveNsd.pdf',width=5,height=4)
plot(x,y,type='l',ylim=c(0,10),xlim=c(0,100),col=cols[1],lwd=2,
  xlab="Species richness",ylab="Mean number of extinctions");
points($(rich_ss[1]),0,pch=16,col=cols[1]);
segments(x0=$(rich_ss[1]),y0=0,x1=$(rich_ss[1]),y1=-1,col=cols[1]);
