...
\end{equation}
For large communities, the number of trophic interactions for a given species $i$ in the Niche Model ($d_i$) is proportional to the niche range $r_i$, where $d_i = r_i*\mathcal{N}$ given $r_i = Xn_i$, where $X \sim \beta(1,1/2C - 1)$ and $n_i \sim \mathcal{U}(0,1)$. %is uniform in [0,1].
