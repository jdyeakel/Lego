...
\rev{While a basal resource is always assumed to be present, (Fig. \ref{fig:model}A)} following the establishment of an autotrophic base, the arrival of mixotrophs (i.e. mixing auto- and heterotrophy) and lower trophic heterotrophs create opportunities for organisms occupying higher trophic levels to invade.
...
% As such, species with increased service interactions and 
...
See Methods and Supporting Information, section I for a complete description of the assembly process. % due to losses from primary extinctions.
...
\subsection*{Section II: Model parameterizations}
...
\noindent \textbf{Assembly without ecosystem engineering} Here we set the average number of modifiers made per species $\lambda = 0$ and the probability of need interactions in the species pool $p_{\rm n}=0.002$.

\noindent \textbf{Structure and dynamics of mutualisms} Again we used the default parameterizations but set $\lambda = 0$, while varying $p_{\rm n} \in [0,0.002]$.

\noindent \textbf{Assembly with ecosystem engineering} Here we used the default parameterizations but varied $\lambda \in [0,2]$ and $p_{\rm n} \in [0,0.002]$.

\subsection*{Section III: Comparison to niche model}
...
\subsection*{Section IV: Measures of generality}
