...
The layers of natural history giving rise to an ecological community can be simplified to a network, where nodes represent species and links represent interactions between them.
...
This minimalist perspective provides insight into community structure, and consequently, the dynamics of species' populations and the stability of the system as a whole (refs).
...
For example, mutualisms form the foundation of mutualistic networks, whereas trophic interactions form the foundation of food webs. %- which are often trophic in one direction and a reproductive service in the other -
More recently, there has been a growing interest in understanding community structure by taking into account diverse interaction types, where multiple unique interactions are included in a multilayer network \cite{Kefi:2016gq}.
However, interactions between two species are inherently compound in nature, such that both species in an interaction experiences different effects.
For example, in a mutualistic interaction, there is often biomass flow in one direction and a reproductive service in the other; in a trophic interaction there is biomass flow from the prey to the predator and this effect is asymmetric.
While this asymmetry is generally not encoded in the structure of ecological networks, it becomes central with description of network dynamics (May, Allesina, Gross, etc).


%Ecosystem engineering
Diverse interactions occur directly between species, but also indirectly through the effect that species have on their environment.
If these environmental effects have timescales longer than that of the instigating species, we call those species ecosystem engineers (ref).
...
Terrestrial plants have altered the stoichiometry of the atmosphere, providing the molecular stores required for respiration (refs).
Elephants root out large saplings and small trees, enabling the formation and maintenance of grasslands (refs) on which grass specialists depend.
...
And while the importance of ecosystem engineers is celebrated, explorations of their effects on community dynamics occupy a very limited role in theory (Hastings refs), and are absent in models of ecological networks.
...
The links of the network that connect both species and objects represent resource, service, and engineering dependencies, respectively.
Following Pillai (REF), we do not track the abundances of entities but only track their presence or absence.
We use this framework to explore the dynamics of ecosystem assembly, where the colonization and extinction of species within a community depends on the constraints imposed by the resource, service, and engineering dependencies connecting both species and objects.
